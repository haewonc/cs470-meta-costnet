...
class Mix4Trainer(BaseTrainer):
    ...
    def train(self):
        ...
        for epoch in range(self.config.total_epoch):
            ...
            print_total('TRAIN', epoch, self.config.total_epoch, self.config.loss, avg_loss, ["BIKE PU", "BIKE DO", "TAXI PU", "TAXI DO"], avg_metrics)
        ...
        torch.save(self.best_model, '../results/saved_models/{}/all.pth'.format(self.save_name))
        with open('../results/saved_models/{}/history_all.json'.format(self.save_name), 'w') as file:
    ...
    def validate(self, epoch, i=None, is_test=False):
        ...
        print_total('TEST' if is_test else 'VALID', epoch, self.config.total_epoch, self.config.loss, avg_loss, ["BIKE PU", "BIKE DO", "TAXI PU", "TAXI DO"], avg_metrics)

    def train_epoch(self, epoch, i=None):
        self.model.train()
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            data = torch.permute(data[:, :, :, :], [0, 3, 1, 2]).to(self.device)  
            target = torch.permute(target[:, :, :, :], [0, 3, 1, 2]).to(self.device)  
            ...
            for j in range(4):
                output[:, j] *= self.data_max[j]
                target[:, j] *= self.data_max[j]
                output[:, j] += self.data_min[j]
                target[:, j] += self.data_min[j]
                
                this_metrics.append(self._eval_metrics(output[:, j], target[:, j])[0])

            total_metrics += np.array(this_metrics)

            print_progress('TRAIN', epoch, self.config.total_epoch, batch_idx, self.num_train_iteration_per_epoch, training_time, self.config.loss, loss.item(), ["BIKE PU", "BIKE DO", "TAXI PU", "TAXI DO"], this_metrics)

        return total_loss, total_metrics

    def validate_epoch(self, epoch, i=None, is_test=False):
        self.model.eval()
        start_time = time.time()
        total_loss = 0
        total_metrics = np.zeros(4)
        ...
        for batch_idx, (data, target) in enumerate(self.test_loader if is_test else self.val_loader):
            ...
            print_progress('TEST' if is_test else 'VALID', epoch, self.config.total_epoch, batch_idx, \
                ...
                self.config.loss, loss.item(), ["BIKE PU", "BIKE DO", "TAXI PU", "TAXI DO"], this_metrics)
