...
class Mix2Trainer(BaseTrainer):
    ...
    def train(self):
        ...
        for i in range(2):
            ...
            for epoch in range(self.config.total_epoch):
                ...
                print_total('TRAIN', epoch, self.config.total_epoch, self.config.loss, avg_loss, ["PU", "DO"], avg_metrics)
    ...
    def validate(self, epoch, i, is_test=False):
        ...
        print_total('TEST' if is_test else 'VALID', epoch, self.config.total_epoch, self.config.loss, avg_loss, ["PU", "DO"], avg_metrics)
    ...
    def train_epoch(self, epoch, i):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            data = torch.permute(data[:, :, :, i*2:(i+1)*2], [0, 3, 1, 2]).to(self.device)  
            target = torch.permute(target[:, :, :, i*2:(i+1)*2], [0, 3, 1, 2]).to(self.device)  
            ...
            for j in range(2):
                output[:, j] *= self.data_max[i*2+j]
                target[:, j] *= self.data_max[i*2+j]
                output[:, j] += self.data_min[i*2+j]
                target[:, j] += self.data_min[i*2+j]
                
                this_metrics.append(self._eval_metrics(output[:, j], target[:, j])[0])

            total_metrics += np.array(this_metrics)

            print_progress('TRAIN', epoch, self.config.total_epoch, batch_idx, self.num_train_iteration_per_epoch, training_time, self.config.loss, loss.item(), ["PU", "DO"], this_metrics)
    ...
    def validate_epoch(self, epoch, i, is_test):
        ...
        for batch_idx, (data, target) in enumerate(self.test_loader if is_test else self.val_loader):
            ...
            print_progress('TEST' if is_test else 'VALID', epoch, self.config.total_epoch, batch_idx, \
                ...
                self.config.loss, loss.item(), ["PU", "DO"], this_metrics)
